...
import com.biantech.spring.samples.mvcdemo.support.RowBoundsArgumentResolver;
...
public class DemoModelControllerTest extends AbstractJUnit4SpringContextTests {
   ...
   @Test
    ...
    public void testShow2() throws Exception {
        ...
        ResultActions ac = mockMvc.perform(post("/demo/show2").contentType(MediaType.APPLICATION_JSON).content("{\"id\":1,\"name\":\"test\"}").accept(MediaType.ALL));
        ac.andExpect(status().isOk());
        ac.andDo(print());
    ...
    public void testShowStatus() throws Exception {
        LOG.info("run");
        ResultActions ac = mockMvc.perform(get("/demo/show_status").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).accept(MediaType.ALL));
        ac.andExpect(status().isOk());
        ac.andDo(print());
        LOG.info("end");

...
/**
 * <code>RawOldControllerTest</code>
 *
...
@WebAppConfiguration
public class RawOldControllerTest extends AbstractJUnit4SpringContextTests {
    ...
    @Test
    public void testRawOld() throws Exception {
        LOG.info("run");
        ResultActions ac = mockMvc.perform(MockMvcRequestBuilders.get("/rawOld").accept(MediaType.ALL));
        ac.andDo(MockMvcResultHandlers.print());
        ac.andExpect(MockMvcResultMatchers.status().isOk());
        LOG.info("end");
    }
    @Test
    public void testRawOld2() throws Exception {
        LOG.info("run");
        ResultActions ac = mockMvc.perform(MockMvcRequestBuilders.get("/rawold").accept(MediaType.ALL));
        ac.andDo(MockMvcResultHandlers.print());
        ac.andExpect(MockMvcResultMatchers.status().isOk());
